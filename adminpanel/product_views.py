...
import os
from products.models import FAQ, ProductCategory, ProductSubCategory, ProductDesign, ProductSize, Product, ProductVariant, ProductDesignAssociation, ProductImage
...
def product_category_create_or_update(request, pk=None):
    ...
    return render(request, 'product/category/product_category_form.html', {'product_category': product_category})

def product_category_list(request):
    product_categories = ProductCategory.objects.all()
    return render(request, 'product/category/product_category_list.html', {'product_categories': product_categories})
...
def product_subcategory_create_or_update(request, pk=None):
    ...
    return render(request, 'product/subcategory/product_subcategory_form.html', 
                  {'product_subcategory': product_subcategory, 'product_categories': product_categories})

def product_subcategory_list(request):
    product_subcategories = ProductSubCategory.objects.all()
    return render(request, 'product/subcategory/product_subcategory_list.html', {'product_subcategories': product_subcategories})
...
def product_designs_list(request):
    ...
    return render(request, 'product/design/product_designs_list.html', {'product_designs': designs})
...
def product_size_create_or_update(request, pk=None):
    ...
    return render(request, 'product/size/product_size_form.html', {'product_size': product_size})

def product_size_list(request):
    product_sizes = ProductSize.objects.all()
    return render(request, 'product/size/product_size_list.html', {'product_sizes': product_sizes})
...
def product_create_or_update(request, pk=None):
    ...
    selected_design_ids = []
    if product:
        selected_design_ids = list(product.design_associations.values_list('design_id', flat=True))
    ...
    if request.method == 'POST':
        try:
            ...
            subcategory = ProductSubCategory.objects.get(id=subcategory_id) if subcategory_id else None
            ...
            messages.success(request, f'Product {"updated" if pk else "created"} successfully!')
...
def get_subcategories(request):
    ...
    subcategories = ProductSubCategory.objects.filter(category_id=category_id).values('id', 'subcategory_name')
...
def product_image_list(request):
    ...
    return render(request, 'product/image/product_image_list.html', {'product_images': product_images})
...
def product_variant_create_or_update(request, pk=None):
    ...
    return render(request, 'product/product_variants/product_variant_form.html', context)

def product_variant_list(request):
    variants = ProductVariant.objects.select_related('product', 'size').all()
    return render(request, 'product/product_variants/product_variant_list.html', {'product_variants': variants})
...
def faq_create_or_update(request, pk=None):
    ...
    if request.method == 'POST':
        ...
        if image:
            existing_faq_with_image = FAQ.objects.filter(category=category, image__isnull=False).exclude(pk=pk).first()
        ...
        if faq:
            ...
            FAQ.objects.create(question=question, answer=answer, category=category, image=image)
            messages.success(request, 'FAQ created successfully!')

        return redirect('faq_list')

    categories = ProductCategory.objects.all()
    return render(request, 'product/faq/faq_form.html', {'faq': faq, 'categories': categories})
